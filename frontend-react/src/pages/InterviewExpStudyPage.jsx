 ...
 * 支持自评掌握度。结构镜像 LearnPage（复用 learn-* 样式），砍掉子话题多卡与探索对话。
 */
...
export default function InterviewExpStudyPage() {
  ...
  const [selfVal, setSelfVal] = useState(0)
  ...
  const loadContent = useCallback(async (qid, action = 'fetch') => {
    ...
    if (action === 'fetch' && cacheRef.current[qid]) {
      ...
      setSelfVal(cacheRef.current[qid].self_mastery ?? 0)
      return
    }
    ...
    try {
      ...
      if (resp.code === 0) {
        cacheRef.current[qid] = resp.data
        setContent(resp.data)
        setSelfVal(resp.data.self_mastery ?? 0)
      } else {
  ...
  async function handleSetSelfMastery(value) {
    if (!activeId) return
    if (content?.self_mastery === value) return
    try {
      const resp = await postExp('/self-mastery', { question_id: activeId, self_mastery: value })
      if (resp.code !== 0) { alert('设置失败: ' + (resp.message || '未知错误')); return }
      setContent(prev => {
        if (!prev) return prev
              ...
              <div className="learn-meta">
                ...
                <span className="learn-self-mastery">
                  ...
                  <input
                    ...
                    value={selfVal} className="lsm-slider"
