...
async def create_tree_from_generate(tree_name: str, requirements: str, db: AsyncSession) -> dict:
    """根据需求描述，LLM 一次生成完整知识树"""
    llm = get_llm(temperature=0.3, max_tokens=8192)
    prompt = TREE_GENERATE_PROMPT.format(tree_name=tree_name, requirements=requirements)
...
# ============================================================
# 语义去重检测：LLM 判断新树名是否与已有树名语义重复
# ============================================================

async def check_duplicate_tree(new_root_id: int, db: AsyncSession) -> dict | None:
    """
    检查新创建的根节点是否与已有根节点语义重复。
    返回: {"duplicate_id": int, "duplicate_name": str} 或 None
    """
    new_root = await db.get(KnowledgeNode, new_root_id)
    if not new_root or new_root.parent_id is not None:
        return None

    new_name = (new_root.name or "").strip()
    if not new_name:
        return None
    ...
    result = await db.execute(
        select(KnowledgeNode).where(
            ...
            KnowledgeNode.id != new_root_id,
        )
    )
    other_roots = result.scalars().all()
    if not other_roots:
        return None

    existing_names = [r.name.strip() for r in other_roots if r.name.strip()]
    if not existing_names:
        return None

    # 先做快速精确匹配（大小写不敏感）
    for r in other_roots:
        if r.name.strip().lower() == new_name.lower():
            return {"duplicate_id": r.id, "duplicate_name": r.name}

    # LLM 语义匹配
    llm = get_llm(temperature=0.0, max_tokens=256)
    names_text = "\n".join(f"- {n}" for n in existing_names)
    prompt = TREE_DUPLICATE_CHECK_PROMPT.format(new_name=new_name, existing_names=names_text)
    ...
    try:
        ...
        if data.get("duplicate") and data.get("matched_name"):
            ...
            # 找到对应的根节点 id
            for r in other_roots:
                if r.name.strip() == matched:
                    return {"duplicate_id": r.id, "duplicate_name": r.name}
            # 名称可能略有出入，再做模糊匹配
            for r in other_roots:
                if r.name.strip().lower() == matched.lower():
                    return {"duplicate_id": r.id, "duplicate_name": r.name}
    except Exception as e:
