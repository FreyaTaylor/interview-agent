...
from backend.services.tree import (
    ...
    create_tree_from_mm, optimize_tree, merge_trees, check_duplicate_tree,
)
...
async def delete_tree_node(
    ...
    # 如果父节点不再有子节点，将其 node_type 改为 leaf
    if parent_id:
...
@router.get("/trees/{root_id}/check-duplicate", summary="语义检测知识树是否重复")
async def check_duplicate_api(
    root_id: int,
    db: AsyncSession = Depends(get_db),
) -> ApiResponse:
    """检查新建的根节点是否与已有知识树语义重复"""
    result = await check_duplicate_tree(root_id, db)
    return ApiResponse.ok(data=result or {})
...
async def merge_trees_api(
    ...
    """将 source 树合并到 target 树，合并后删除 source"""
    try:
