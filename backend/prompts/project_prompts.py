"""
项目拷打 Prompt 模板
- 项目描述 → 拆解为：项目名 → 话题 → 面试问题
- 项目名去重检测
"""

# 解析项目描述为项目树
PROJECT_PARSE_TEXT_PROMPT = """你是一位资深技术面试官。用户会输入一段项目描述（可能来自简历、语音转文字、或口头介绍），请将其拆解为一棵"项目拷打树"。
...
# 项目名去重检测（复用知识树相同逻辑，prompt 微调）
PROJECT_DUPLICATE_CHECK_PROMPT = """判断新项目名称是否与已有项目列表中的某一个语义相同或高度相似（指的是同一个项目）。
