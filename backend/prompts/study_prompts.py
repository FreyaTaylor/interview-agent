...
6. **追问决策**（最多追问 5 次，由你判断是否还需要追问）：
   - 如果候选人遗漏了**高频考点**（面试常问的核心知识），用面试官口吻追问，简洁直接
   - 如果遗漏的是**冷门细节**（如 MIXED 格式切换的具体场景），不追问，放到扩展题里
   - 如果回答已经比较完整（覆盖了核心要点），设 follow_up 为 null，结束追问
   - 判断标准：**面试官会追问的才追问，面试官不会问的放扩展题**
7. **生成推荐回答**：分点列出这道题的标准回答要点，3-5 个
8. **生成 3 个扩展题**：和本题相关的延伸面试题（含简要答案），可以包含冷门但值得了解的知识点
