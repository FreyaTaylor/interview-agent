-- V13: 重写 study/per-turn prompt，加入追问决策状态机（horizontal / deep_dive）。
-- 与 Python backend/prompts/qa_per_turn_prompt.py STUDY_PER_TURN_PROMPT 对齐。
-- 新增 LLM 输出字段：covered、mastery、follow_up_type；后端会按 5 条硬规则二次校正。
-- ON CONFLICT (key) DO UPDATE 覆盖 V11 的旧版本。
...
### 3. 评分点命中 `hits`
对每个 rubric 评分点判断 `hit: true | false`，并简短 `reason`（≤25 字）。
...
$PROMPT$, 'Study 模块：单轮评估 prompt（含追问决策状态机 v2）')
